...
uint8_t debug_on = 0; // Задание начальных статусов режимов работы
...
static void debug_ch_process(uint8_t ch) //Блок обработки нажатий кнопок в отладке.
{
	if (ch == 'o') {
...
static void usart_process(void) //Блок чтения сообщений из USART
{
	uint8_t ch = 0;
	...
	if (!debug_on) {
		...
		if (ch == 'O') {
			...
			if (debug_on_cnt++ > 10) { //Счетчик нажатий "O" для перехода в отладку.
...
void print_sniffer(void) // Экран снифера
{
	char buf[200];
...
void print_debug(void) // Экран отладчика
{
	char buf[100];
	...
	const char * scar = "";
	enum e_car_t car = car_get_car(); // Перечислитель для выбора авто в отладке.
	switch (car) {
		case e_car_subaru_sj:
			scar = "SUBARU FORESTER SJ 13MY";
			break;
//		case e_car_subaru_wrx:
//			scar = "SUBARU WRX 16MY";
//			break;
//		case e_car_anymsg:
//			scar = "Any Msg";
//			break;
	...
	const char * scb = "";
	enum e_canbox_t cb = conf_get_canbox(); // Перечислитель для выбора UART протокола в отладке
	switch (cb) {
...
static void gpio_process(void) // Реакция на входа GPIO
{
	uint8_t acc = car_get_acc();
//	uint8_t ign = car_get_ign();
//	uint8_t park_lights = car_get_park_lights();
...
//	uint8_t acc = car_get_acc(); Закомментил, т.к. неиспользуется в этой функции
	...
	while(1) {
		...
		if (timer.flag_1000ms) {
			...
			if (ms_can_stop_counter > 10) { //Счетчик секунд до засыпания при отсутствии CAN пакетов.
