...
#include <stdlib.h>
#include <errno.h>
			// Описания протоколов связи с ГУ по USART
#include "hw_usart.h"
#include "conf.h"
#include "car.h"
...
void canbox_raise_vw_door_process(void)
{
	...
	if (conf_get_car() == e_car_subaru_sj) {
...
void canbox_raise_vw_vehicle_info(void)
{
	...
	uint8_t t12 = car_get_fuel_level(); // была ошибка в изначальном коде с low_fuel
...
void canbox_raise_vw_ac_process(void)
{
	...
	if (fanspeed)
		buf[1] |= (uint8_t)fanspeed & 0x07;
	if (l_temp)
		buf[2] = ((uint8_t)l_temp);
	if (r_temp)
		buf[3] = ((uint8_t)r_temp);

/*	uint8_t speed = scale(fanspeed, 0x00, 0x07, 0x00, 0x07);
	buf[1] |= fanspeed & 0x07;
